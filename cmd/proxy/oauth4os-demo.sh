...
cmd_shell() {
  local HISTFILE="${CONFIG_DIR}/shell_history"
  local SHELL_CMDS="login logout refresh register revoke rotate search sql tail watch stream services indices clients tokens sessions keys stats export dashboard bookmark history config alias completion status token whoami profile ping latency alerts audit diff top env install-man changelog version tutorial policy backup restore metrics inspect replay userinfo health help quit exit"

  # Enable readline if available
  if [ -t 0 ]; then
    set -o emacs 2>/dev/null
    # Load history
    [ -f "$HISTFILE" ] && history -r "$HISTFILE" 2>/dev/null
    # Tab completion
    _shell_complete() {
      local cur="${COMP_WORDS[COMP_CWORD]}"
      if [ "$COMP_CWORD" -eq 1 ]; then
        COMPREPLY=($(compgen -W "$SHELL_CMDS" -- "$cur"))
      elif [ "${COMP_WORDS[1]}" = "policy" ] && [ "$COMP_CWORD" -eq 2 ]; then
    ...
    complete -F _shell_complete oauth4os 2>/dev/null
    bind 'set show-all-if-ambiguous on' 2>/dev/null
    bind 'TAB:complete' 2>/dev/null
  fi

  echo -e "${BOLD}🔐 oauth4os interactive shell${NC}"
  echo -e "  Type commands without 'oauth4os-demo' prefix. Tab to complete. 'quit' to exit.\n"

  # Show connection status
  local status_code=$(curl -s -o /dev/null -w '%{http_code}' "${PROXY}/ready" 2>/dev/null)
  if [ "$status_code" = "200" ]; then
    echo -e "  ${GREEN}● Connected to ${PROXY}${NC}"
  else
    echo -e "  ${YELLOW}○ Proxy at ${PROXY} (status: ${status_code:-timeout})${NC}"
  fi

  # Show auth status
  local tok=$(get_token 2>/dev/null)
  if [ -n "$tok" ]; then
    local sub=$(echo "$tok" | cut -d. -f2 | tr '_-' '/+' | awk '{l=length%4;if(l==2)$0=$0"==";else if(l==3)$0=$0"=";print}' | base64 -d 2>/dev/null | jq -r '.sub // .client_id // "unknown"' 2>/dev/null)
    echo -e "  ${GREEN}● Authenticated as ${sub}${NC}\n"
  else
    echo -e "  ${YELLOW}○ Not authenticated (run: login)${NC}\n"
  fi

  while true; do
    local input
    if ! read -e -p "$(echo -e "${CYAN}oauth4os>${NC} ")" input; then
      echo; break
    fi
    # Skip empty
    [ -z "$input" ] && continue
    ...
    [ -d "$CONFIG_DIR" ] && history -w "$HISTFILE" 2>/dev/null
    ...
    case "$cmd" in
      quit|exit) echo -e "${GREEN}Bye!${NC}"; break ;;
      clear) clear ;;
      !*) # Shell escape
        eval "${input#!}" ;;
      *)
        # Re-dispatch through main case
        if echo "$SHELL_CMDS" | grep -qw "$cmd"; then
          # Build args array
          ...
          case "$CMD" in
            ...
            version|changelog) cmd_changelog ;;
            help)       echo "$SHELL_CMDS" | tr ' ' '\n' | pr -t -4 -w 72 ;;
            *)          echo -e "${YELLOW}Unknown: $CMD${NC}" ;;
          esac
        else
          echo -e "${YELLOW}Unknown command: $cmd${NC}  (type 'help' for commands)"
        fi
        ;;
    esac
  done
